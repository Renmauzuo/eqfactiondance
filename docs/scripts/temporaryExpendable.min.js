...
$(function () {
  var unraisableFactions = []; //Candidate factions are Important or Unimportant factions that can be increased.
  ...
  do {
    factionsToAdd = candidateFactions.filter(function (faction) {
      if (faction.factionsLowered) {
        for (var i = 0; i < faction.factionsLowered.length; i++) {
          var factionLowered = factionList[faction.factionsLowered[i]]; //If lowered faction is neither expendable nor temporarily expendable this faction isn't one to add (yet)
    ...
    temporarilyExpendableFactions = temporarilyExpendableFactions.concat(factionsToAdd); //Remove added factions from candidate list
...
//# sourceMappingURL=temporaryExpendable.min.js.map
